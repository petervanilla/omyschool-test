...
const replacementSets = `const BINGO_SETS = [
...
const startIndex = code.indexOf('const ALL_BINGO_ITEMS = [');
const endIndex = code.indexOf('];', startIndex) + 2;

code = code.substring(0, startIndex) + replacementSets + code.substring(endIndex);

code = code.replace(
  'const shuffled = [...ALL_BINGO_ITEMS].sort(() => 0.5 - Math.random());',
